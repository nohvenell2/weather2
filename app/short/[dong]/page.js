...
export default function Short({params}){
    const [dataShort, setData] = useState(null);
    const [error, setError] = useState(null);
    const dong = decodeURIComponent(params.dong)
    const title = `단기 예보 - ${dong}`
    useEffect(()=>{
        const fetchData = async () =>{
            try{
                const response = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL}/api/weather-data/get-short?dong=${dong}`)
                if (!response.ok){
